...
def countCellsWithSelectivity_or(infoList1, infoList2, results, plotOn=True,infoThreshold = 1.0):
    ## count number of cells developed to be selective to a stimulus    
    nObj = np.shape(results)[0];
    nTrans = np.shape(results)[1];
...
def persistentHomology(weights):
    ...
    layers = np.zeros((64,nBins))+0.5;

    for th in range(nBins):
        weights_th = np.copy(weights);
        weights_th[weights<(1/nBins)*th] = 0
        
        for i in range(64):
#             layers[i,th] = np.mean(weights_th[:784,i])-np.mean(weights_th[784:,i]);
            if np.mean(weights_th[:784,i])>np.mean(weights_th[784:,i]):
                layers[i,th]= 1;
            elif np.mean(weights_th[:784,i])<np.mean(weights_th[784:,i]):
                layers[i,th]= 0;
